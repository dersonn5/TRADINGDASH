...
@dataclass
class SilverBulletXAUConfig:
    killzone_start: time = time(10, 0)  # EST — NY AM session
    killzone_end: time = time(11, 0)    # EST
    max_trades_per_day: int = 2
    min_rr: float = 2.0
    sl_buffer_usd: float = 0.5          # $0.50 buffer para XAU
    tick_size: float = 0.1              # XAU tick = $0.10
    require_daily_bias: bool = True
    require_premium_discount: bool = True
    require_sweep: bool = False          # Sweep prévio de PDH/PDL obrigatório
    require_mss: bool = True            # MSS confirmando direção obrigatório
...
class SilverBulletXAU(Strategy):
    name = "silver_bullet_xau"
    symbol = "XAUUSD"
    timeframe_entry = "5m"
    timeframe_structure = "15m"
    timeframe_bias = "1h"

    def __init__(self, config: SilverBulletXAUConfig = None):
        self.config = config or SilverBulletXAUConfig()
    ...
    def _check_sweep(self, df_5m: pd.DataFrame, pdh: float, pdl: float, current_time: datetime) -> Tuple[bool, bool]:
        ...
        start_check = pd.to_datetime(f"{day_str} 02:00:00").tz_localize('America/New_York')
    ...
    def evaluate(
        ...
        if not self._gate("bias_not_neutral", bias_ok, bias,
                          "PO3 retornou NEUTRAL (close não em top/bottom 30% do range)"):
            return None

        day_start = pd.to_datetime(current_time.strftime("%Y-%m-%d 00:00:00")).tz_localize('America/New_York')
        ...
        london_bias = self._detect_london_manipulation(df_5m, asian_high, asian_low, current_time)
        if not self._gate("london_bias_active", london_bias != "NONE", london_bias, f"London did not sweep Asian Range"):
            return None
        ...
        if fvg_bullish:
            ...
            if not self._gate("sweep_present_bullish", sweep_ok,
                              ...
                              "sem sweep do Asian Low desde 04:00 EST"):
                return None
            ...
            if m1_conf is not None:
                ...
                if df_1m is not None and not df_1m.empty:
                    ...
                    stop_loss = fvg_bot_m5 - self.config.sl_buffer_usd
                    sl_distance = abs(entry_price - stop_loss)
                    if sl_distance < self.config.min_sl_distance_usd:
                        stop_loss = entry_price - self.config.min_sl_distance_usd
                    take_profit = entry_price + (abs(entry_price - stop_loss) * self.config.min_rr)
            ...
            reason = (f"Silver Bullet BULLISH XAU. Sweep PDL OK, MSS confirmado + Displacement. "
            ...
            if not self._gate("sweep_present_bearish", sweep_ok,
                              ...
                              "sem sweep do Asian High desde 04:00 EST"):
                return None
            ...
            if m1_conf is not None:
                ...
                if df_1m is not None and not df_1m.empty:
                    ...
                    entry_price = fvg_ce_m5
                    stop_loss = fvg_top_m5 + self.config.sl_buffer_usd
                    sl_distance = abs(stop_loss - entry_price)
                    if sl_distance < self.config.min_sl_distance_usd:
                        stop_loss = entry_price + self.config.min_sl_distance_usd
                    take_profit = entry_price - (abs(stop_loss - entry_price) * self.config.min_rr)
            ...
            self._last_signal_ts = current_time
            reason = (f"Silver Bullet BEARISH XAU. Sweep PDH OK, MSS confirmado + Displacement. "
                       f"M5 [{fvg_bot_m5:.2f}-{fvg_top_m5:.2f}]. Entry ({entry_price:.2f}).")
            return Signal(
...
if __name__ == "__main__":
    config = SilverBulletXAUConfig()
    strategy = SilverBulletXAU(config)
